...
deployment in tandem with the usethis, devtools, and sinew packages. Features
include:  

* Generate yaml skeletons for the pkgdown package  
* Write Rnw files for Sweave to incorporate static pdf vignettes into a package  
...
What if I want to author a new function that prints both `things I love` and 
`things I like` in a single function call?   

I can use `easyBakeOven`'s `make_args` functions to return arguments and their values 
for `love()` and `like()` to copy-and-paste into the new function.   
...
I can then copy-and-paste the results from the above calls to `make_args` to 
author `like_and_love()`. Starting with the default values for the arguments for  
...
Please note that the suzyBakeOven project is released with a [Contributor Code
