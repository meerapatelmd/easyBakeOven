...
process_gmail_filters_xml <-
        function(xml_path) {
gmail_filters_xml <- xml2::read_xml(x = "~/Desktop/mailFilters.xml")
...
gmail_filter_pivoted %>% dplyr::select(all_of(identifier_fields), all_of(logic_fields), all_of(consequence_fields)) %>%
        arrange(label)
